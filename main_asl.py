...
class ImageDataset(Dataset):
    def __init__(self, data_file, label_file=None, flip=0):
        """
        Load data. If label_file is provided, load labels.
        If flip=1, flip the x and y coordinates (only the first two values in the last dimension).
        """
        ...
        if flip == 1:
            # Only flip the first two dimensions in the last axis (x and y)
            self.data[..., 0:2] = -self.data[..., 0:2] + 1
...
def main(mode="train"):
    ...
    train_data_file = "data/train_combined_data_frames.npy"
    train_label_file = "data/train_combined_label.pkl"
    test_data_file1 = "data/val_handshape_dominant.npy"
    test_data_file2 = "data/val_handshape_non_dominant.npy"
    output_dir = "save"
    save_json1 = "json/dom_top10_results.json"
    save_json2 = "json/non_dom_top10_results.json"
    ...
    schedulers = [StepLR(optimizer, step_size=150, gamma=0.1) for optimizer in optimizers]
    ...
    if mode == "train":
        ...
        test_dataset = ImageDataset(test_data_file1)
        ...
        test_dataset = ImageDataset(test_data_file2, flip=1)

        test_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=False)

        predictions_top10 = predict_ensemble(models, test_loader, device)

        with open(save_json2, 'w') as f:
            json.dump(predictions_top10, f)
